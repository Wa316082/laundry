            ...
            <form action="{{ route('customer.update', $customer->id) }}" method="POST" class="row">
                ...
                <div class="col-md-6 col-12 mb-4">
                    ...
                    <input type="contact" id="contact" name="contact" class="form-control"
                        placeholder="Customer contact"value="{{ old('contact', $customer->contact) }}">
                    @error('contact')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-md-6 col-12 mb-4">
                    ...
                    <input type="email" id="email" name="email" class="form-control"
                        placeholder="Customer email"value="{{ old('email', $customer->email) }}">
                    @error('email')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-md-6 col-12 mb-4">
                    ...
                    <input type="address" id="address" name="address" class="form-control"
                        placeholder="Customer Address"value="{{ old('address', $customer->address) }}">
                    @error('address')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
